...
export class CombatImplementor implements Combat{
    findTarget(unit: Unit, enemyArmy: Army): Unit{
        const enemySoldiers = enemyArmy.units.filter(unit => unit.getType() === 'Soldier' && unit.isAlive);
        const enemyArtillery = enemyArmy.units.filter(unit => unit.getType() === 'Artillery unit' && unit.isAlive);
        const enemyAirforce = enemyArmy.units.filter(unit => unit.getType() === 'Airforce unit' && unit.isAlive);
    ...
    performCombatWithArmyUnit(index: number, attackingArmy: Army, defendingArmy: Army): Army{
        const target = this.findTarget(attackingArmy.units[index], defendingArmy);
        if(target !== undefined){
            const attackedTarget = this.attackEnemy(attackingArmy.units[index], target);
    ...
    performCombat(army1: Army, army2: Army): string{
        let index1 = 0;
        let index2 = 0;
        while(army1.units.find(unit => unit.isAlive) && army2.units.find(unit => unit.isAlive)){
